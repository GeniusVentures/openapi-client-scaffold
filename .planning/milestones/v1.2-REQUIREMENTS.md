...
shipped: 2026-08-24
note: Requirements were delivered and verified in phases 8-11; several
  checkboxes/traceability rows were not ticked at execution time and were
  corrected during archival (WIDG-32..34, 37-39 verified in Phase 9
  VERIFICATION; WIDG-44/45 verified in Phase 11 VERIFICATION).
---
...
- [x] **WIDG-32**: `ScaffoldStreamingRichText` renders incrementally-updated rich text without rebuilding the full response
- [x] **WIDG-33**: `ScaffoldStreamingRichText` supports inline source/citation markers with expandable source slots and streaming cursor state
- [x] **WIDG-34**: `ScaffoldStreamingRichText` exposes response-action slots (copy, retry, rate, follow-up) and accessibility announcements that don't reread the whole answer per token
...
- [x] **WIDG-37**: `ScaffoldCodeBlock` renders syntax-highlighted code spans with line numbers, language/filename header, and copy action
- [x] **WIDG-38**: `ScaffoldCodeBlock` supports horizontal scrolling, streamed line insertion, and reduced-motion behavior
- [x] **WIDG-39**: `ScaffoldSelectionActions` wraps selectable content and reports `onSelectionChanged(TextSelection, String)` with an anchored action toolbar
...
- [x] **WIDG-44**: Each new atom ships with widget tests, a demo in `example/`, and barrel export (same bar as v1.1)
- [x] **WIDG-45**: Beautiful UI coverage check — all 19 components demonstrably composable from shipped scaffold atoms (7 ready + 8 thin + 4 primitive-enabled)
